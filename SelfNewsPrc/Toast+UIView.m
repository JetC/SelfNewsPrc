...
- (void)makeToastActivity:(id)position {
    ...
    UIView *activityView = [[[UIView alloc] initWithFrame:CGRectMake(0, 0, CSToastActivityWidth, CSToastActivityHeight-5)] autorelease];
    ...
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            if (IOSSystemVersion >= 7) {
                if (self.bounds.size.height == SCREEN_HEIGHT || self.bounds.size.height == SCREEN_HEIGHT - kNavigationBarWithStatusBarHeight) {
                    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding - kTabBarHeight);
                }
            }
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (imageView!=nil && titleLabel == nil && messageLabel != nil) {
        messageLabel.frame = CGRectMake(100,21, 300, 60);
    }
